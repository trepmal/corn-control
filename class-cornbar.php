 ...
 * 
 * @see https://github.com/wp-cli/php-cli-tools/blob/master/lib/cli/progress/Bar.php
 *
...
class CornBar extends Bar {
	...
	public function display($finish = false) {
		...
		$size = $osize = Shell::columns();
		$size -= strlen($msg . $timing);
		if ( $size < 0 ) {
			$size = 0;
		}

		$graphic_bar_len= floor($_percent * $size);
		$bar = str_repeat('X', $graphic_bar_len) . $this->_bars[1];
		...
		// emojis have (seemingly) a physical width of 2 characters in terminal, so replace
		// every 2 `X` with 1 emoji and pad any difference
		$bar = preg_replace_callback( '/(X*)(>?)/', function( $matches ) use( $bar_0 ) {
			$xs =$matches[1];
			$x_len = round( strlen($xs)/2, PHP_ROUND_HALF_UP );
			#var_dump( strlen( $xs ), $x_len );
			return str_repeat( $bar_0, $x_len ) . $matches[2] . ( strlen( $xs )%2 ? ' ' : '' );
		}, $bar );
